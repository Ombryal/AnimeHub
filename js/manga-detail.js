/**
 * manga-detail.js - Handles manga detail page with user list status and list editor
 */
...
async function loadManga() {
    ...
    initEditor(); // set up event listeners
}
...
function renderMediaDetails(m, list) {
    ...
    if (list && list.status) {
        let btnText = '';
        switch (list.status) {
            case 'CURRENT': btnText = 'READING'; break;
    ...
    // Statistics grid (adapted for manga)
    ...
    statsGrid.innerHTML = `
        ...
        ${renderStat('fa-book-open', 'Total Chapters', m.chapters || '??')}
        ${renderStat('fa-tv', 'Format', m.format)}
        ${renderStat('fa-book-open', 'Source', 'N/A')}  <!-- optional, could be source field -->
...
// ---- List Editor Bottom Sheet (same logic as anime) ----
function openListEditor() {
    ...
    totalSpan.innerText = ` / ${mediaData.chapters || '?'}`;
...
async function saveListEntry() {
    ...
    if (mediaData.chapters && progress > mediaData.chapters) progress = mediaData.chapters;
...
function initEditor() {
    ...
    if (plus) plus.onclick = () => {
        let val = parseInt(progressInput.value) || 0;
        if (mediaData.chapters && val < mediaData.chapters) progressInput.value = val + 1;
        else if (!mediaData.chapters) progressInput.value = val + 1;
    };
}
